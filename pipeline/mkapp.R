...
mkdata <- function(file, agecut_greater = 4, var.start = "start.dementia", varlist = c('SEX', 'AGE_GROUP', 'MI', 'CHF', 'PVD', 'CD', 'DE', 'CPD', 
                                                                                                                 ...
                                                                                                                 'AH', 'Pre_Statin', 'Pre_Metformin', 'Pre_Aspirin', 'Pre_NSAIDs_COX2', 'Pre_Clopidogrel',  'EXPCON', 'CCI')){
...
matchfun <- function(filedir = "/home/whe/period30", filenames = list.files("/home/whe/period30"), agecut_greater = 4, var.start = "start.dementia", 
                     varlist = c('SEX', 'AGE_GROUP', 'MI', 'CHF', 'PVD', 'CD', 'DE', 'CPD', 'RD', 'PUD', 'MLD', 'DWOCC', 'DWCC', 'HOP', 'AMILALEMNOS', 'MOSLD', 'MST', 
                                 'AH', 'Pre_Statin', 'Pre_Metformin', 'Pre_Aspirin', 'Pre_NSAIDs_COX2', 'Pre_Clopidogrel',  'EXPCON', 'CCI'),
                     wo = 180, caliper = 0.2){
  
  ## Load data: Use mkdata
  dlist.original <- mclapply(file.path(filedir, filenames), mkdata, agecut_greater = agecut_greater, var.start = var.start, varlist = varlist)
  names(dlist.original) <- filenames
  
  ## Save No match/match data, Sample info, label info
  dinfo <- mclapply(seq_along(dlist.original), function(i){
    ## washout 
...
for (oc in c("dementia", "vasculardementia", "cerebralinfarction", "osteoporosis", "ovf", "ohf")){
  ...
  ## Run matchfun
  dinfo <- matchfun(filedir = "/home/whe/period30", filenames = list.files("/home/whe/period30"), 
